...
BudgetHouseholdDvmtSpecifications <- list(
  ...
  Set = items(
    item(
      ...
      ISELEMENTOF = "",
      SIZE = 0,
      DESCRIPTION = "Household income adjusted upward to account for parking cash out participation and pay-as-you-drive insurance participation"
    ),
    item(
      NAME = "NoBudgetDvmt",
      TABLE = "Household",
      ...
      ISELEMENTOF = "",
      SIZE = 0,
      DESCRIPTION = "Average daily vehicle miles traveled by the household in autos or light trucks without household budget limitations"
    ),
    item(
      NAME = "Dvmt",
      TABLE = "Household",
...
BudgetHouseholdDvmt <- function(L, M) {
  ...
  NoBudgetDvmt_Hh <- local({
    L$ReduceDvmt$Year$Household$Dvmt <- Dvmt_Hh
    M$ReduceDvmt(L$ReduceDvmt)$Year$Household$Dvmt
  })
  ...
  Out_ls$Year$Household <- list(
    AdjIncome = AdjIncome_Hh,
    NoBudgetDvmt = NoBudgetDvmt_Hh,
    Dvmt = Adj_ls$Dvmt_Hh,
