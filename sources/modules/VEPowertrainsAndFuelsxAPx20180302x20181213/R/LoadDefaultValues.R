...
#----------------------------------------------------------------------
#Define function to identify whether data is present for required years
#----------------------------------------------------------------------
checkRequiredYears <-
  function(Years_,
           FileName,
           RequiredYears_ = c(1990, 2000, 2005, 2010, 2020, 2030, 2040, 2050)) {
    if (!all(RequiredYears_ %in% Years_)) {
      stop(paste0(
        "File ", FileName, " must have values for the years ",
...
checkRequiredYears(Years_ = CarSvcFuel_df$Year, FileName = "carsvc_fuel.csv")
#Add to PowertrainFuelDefaults_ls and clean up
PowertrainFuelDefaults_ls$CarSvcFuel_df <- CarSvcFuel_df
rm(Inp_ls, AutoNames_, AutoCols_, LtTrkNames_, LtTrkCols_, CarSvcFuel_df)
...
#Check whether all years are present
checkRequiredYears(Years_ = ComSvcFuel_df$Year, FileName = "comsvc_fuel.csv")
#Add to PowertrainFuelDefaults_ls and clean up
PowertrainFuelDefaults_ls$ComSvcFuel_df <- ComSvcFuel_df
rm(Inp_ls, AutoNames_, AutoCols_, LtTrkNames_, LtTrkCols_, ComSvcFuel_df)
...
#Check whether all years are present
checkRequiredYears(Years_ = HhFuel_df$Year, FileName = "hh_fuel.csv")
#Add to PowertrainFuelDefaults_ls and clean up
PowertrainFuelDefaults_ls$HhFuel_df <- HhFuel_df
rm(Inp_ls, AutoNames_, AutoCols_, LtTrkNames_, LtTrkCols_, HhFuel_df)
...
LdvBiofuelMix_df <-
  processEstimationInputs(
    ...
    "LoadDefaultValues.R")
#Check whether all years are present
checkRequiredYears(Years_ = LdvBiofuelMix_df$Year, FileName = "ldv_biofuel_mix.csv")
#Add to PowertrainFuelDefaults_ls and clean up
...
LdvPowertrainCharacteristics_df <-
  processEstimationInputs(
    ...
    "LoadDefaultValues.R")
#Check whether all years are present
checkRequiredYears(Years_ = LdvPowertrainCharacteristics_df$ModelYear,
                   FileName = "ldv_powertrain_characteristics.csv",
                   RequiredYears = 1975:2050)
#Add to PowertrainFuelDefaults_ls and clean up
...
HhPowertrain_df <-
  processEstimationInputs(
    ...
    "LoadDefaultValues.R")
#Check whether all years are present
checkRequiredYears(Years_ = HhPowertrain_df$ModelYear,
                   FileName = "hh_powertrain_prop.csv",
                   RequiredYears = 1975:2050)
...
#Check whether all years are present
checkRequiredYears(Years_ = CarSvcPowertrain_df$Year,
                   FileName = "carsvc_powertrain_prop.csv")
#Check that powertrain proportion are 0 when powertrain characteristics are NA
Msg_ <- character(0)
for (ty in c("Auto", "LtTrk")) {
...
rm(Inp_ls, AutoNames_, AutoCols_, LtTrkNames_, LtTrkCols_, Msg_, ty,
...
#Check whether all years are present
checkRequiredYears(Years_ = ComSvcPowertrain_df$Year,
                   FileName = "comsvc_fuel.csv")
#Check that powertrain proportion are 0 when powertrain characteristics are NA
Msg_ <- character(0)
for (ty in c("Auto", "LtTrk")) {
  for (pt in c("Icev", "Hev", "Bev")) {
...
#Check whether all years are present
checkRequiredYears(Years_ = HvyTrkFuel_df$Year,
                   FileName = "hvytrk_fuel.csv")
#Add to PowertrainFuelDefaults_ls and clean up
PowertrainFuelDefaults_ls$HvyTrkFuel_df <- HvyTrkFuel_df
rm(Inp_ls, Names_, Cols_, HvyTrkFuel_df)
...
HvyTrkBiofuelMix_df <-
  processEstimationInputs(
    ...
    "LoadDefaultValues.R")
#Check whether all years are present
checkRequiredYears(Years_ = HvyTrkBiofuelMix_df$Year,
                   FileName = "hvytrk_biofuel_mix.csv")
#Add to PowertrainFuelDefaults_ls and clean up
...
HvyTrkPowertrainCharacteristics_df <-
  processEstimationInputs(
    ...
    "LoadDefaultValues.R")
#Check whether all years are present
checkRequiredYears(Years_ = HvyTrkPowertrainCharacteristics_df$Year,
                   FileName = "hvytrk_powertrain_characteristics.csv")
#Add to PowertrainFuelDefaults_ls and clean up
...
#Check whether all years are present
checkRequiredYears(Years_ = HvyTrkPowertrain_df$Year,
                   FileName = "hvytrk_powertrain_prop.csv")
#Check that powertrain proportion are 0 when powertrain characteristics are NA
Msg_ <- character(0)
for (pt in c("Icev", "Hev", "Bev")) {
...
rm(Inp_ls, Names_, Cols_, Msg_, pt, HvyTrkPowertrain_df)
...
TransitFuel_df[,Cols_] <-
  sweep(TransitFuel_df[,Cols_], 1, rowSums(TransitFuel_df[,Cols_]), "*")
#Check whether all years are present
checkRequiredYears(Years_ = TransitFuel_df$Year,
                   FileName = "transit_fuel.csv")
#Add to PowertrainFuelDefaults_ls and clean up
PowertrainFuelDefaults_ls$TransitFuel_df <- TransitFuel_df
rm(Inp_ls, Names_, Cols_, TransitFuel_df)
...
TransitBiofuelMix_df <-
  processEstimationInputs(
    ...
    "LoadDefaultValues.R")
#Check whether all years are present
checkRequiredYears(Years_ = TransitBiofuelMix_df$Year,
                   FileName = "transit_biofuel_mix.csv")
#Add to PowertrainFuelDefaults_ls and clean up
...
TransitPowertrainCharacteristics_df <-
  processEstimationInputs(
    ...
    "LoadDefaultValues.R")
#Check whether all years are present
checkRequiredYears(Years_ = TransitPowertrainCharacteristics_df$Year,
                   FileName = "transit_powertrain_characteristics.csv")
#Add to PowertrainFuelDefaults_ls and clean up
...
TransitPowertrain_df[,Cols_] <-
  sweep(TransitPowertrain_df[,Cols_], 1,
        rowSums(TransitPowertrain_df[,Cols_]), "*")
#Check whether all years are present
checkRequiredYears(Years_ = TransitPowertrain_df$Year,
                   FileName = "transit_powertrain_prop.csv")
#Check that powertrain proportion are 0 when powertrain characteristics are NA
Msg_ <- character(0)
...
rm(Inp_ls, Names_, Cols_, Msg_, ty, pt, TransitPowertrain_df)
