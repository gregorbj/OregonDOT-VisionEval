...
calcDensityDistribution <-
  function(Bz_df,
    ...
    shiftRight <- function(DenDist_) {
      Last <- length(DenDist_)
      ShiftDenDist_ <- c(0, unname(DenDist_[-Last]))
      ShiftDenDist_[Last] <- ShiftDenDist_[Last] + DenDist_[Last]
      names(ShiftDenDist_) <- names(DenDist_)
      ShiftDenDist_
    }
    #FUNCTION TO SHIFT A DENSITY DISTRIBUTION TO THE LEFT
    shiftLeft <- function(DenDist_) {
      ShiftDenDist_ <- c(DenDist_[-1], 0)
      ShiftDenDist_[1] <- ShiftDenDist_[1] + DenDist_[1]
      names(ShiftDenDist_) <- names(DenDist_)
      ShiftDenDist_
    }
